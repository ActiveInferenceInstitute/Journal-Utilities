...
### 2. Handle User-Requested Dependencies

For any packages requested by the user that are not in this template or the available skills:

1. **Look up current versions**: Use WebSearch to find the latest stable version on PyPI
   - Search: "pypi {package-name} latest version"
   - Verify the package exists and is actively maintained

2. **Add version constraints**: Include packages in pyproject.toml with proper versioning
   - Use `>=` constraints for the current stable version (e.g., `"package-name>=1.2.3"`)
   - For major packages with good stability, use minor version pinning (e.g., `"package>=1.2.0"`)

3. **Categorize correctly**:
   - Production dependencies → `dependencies = []`
   - Development/testing tools → `[project.optional-dependencies] dev = []`
...
### 4. Generate FastAPI Project Structure
...
### 5. Generate Django Project Structure
...
### 6. Generate Python Library Structure
