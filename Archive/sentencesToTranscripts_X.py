...
charsPerCaption = 40    # cloned from AssemblyAI default - allow override!
...
"""
def writeToSrt(textOut, lineCount, startTime, endTime):
    myLineCount = lineCount
    textRem   = textOut.strip()
    charsRem  = len(textRem)
    timeRem   = endTime - startTime
    ...
    if charsRem > 0:
        ...
        srtStart = ToSRTTime(startTime)
        srtEnd   = ToSRTTime(endTime)
        timeLine  = srtStart + " --> " + srtEnd
        srtPubF.write(timeLine)
        srtPubF.write("\r\n")
        srtPubF.write(textRem)
        srtPubF.write("\r\n")
        srtPubF.write("")
    ...
    for pCount, sentRow in enumerate(allSents):
        if pCount == 0:
            ...
            if len(speaker)>0 or textLen > 0:
                ...
                #
            #
            if textLen > 0:     # SRT written out for each non-empty sentence. 
