...
class DatabaseClient:
    ...
    async def connect(self) -> None:
        ...
        logging.info(f"Connected to database at {self.config.url}")
        ...
        if result and len(result) > 0:
            for audit_record in result:
                ...
                if session_name:
                    try:
                        # Delete the session
                        sessions_to_delete = await db.query(
                            f"SELECT * FROM session WHERE session_name = '{session_name}'"
                        )
                        if sessions_to_delete and len(sessions_to_delete) > 0:
                            for session in sessions_to_delete:
                                await db.query(f"DELETE {session['id']}")
