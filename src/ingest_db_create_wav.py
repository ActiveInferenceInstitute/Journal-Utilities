...
async def copy_files_to_journal(output_dir, journal_repo_dir, db_url, db_user, db_password, db_name, db_namespace):
    ...
    async with AsyncSurreal(db_url) as db:
        ...
        await db.use(db_namespace, db_name)
        result = await db.query("SELECT * FROM session WHERE transcribed AND metadata_filename is NONE AND prose_filename is NONE AND workingcopy_filename is NONE AND category = 'Insights' AND from_coda_csv;")
        for session in result:
            session_id = session['id']
